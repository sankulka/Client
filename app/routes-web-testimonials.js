...
var server = 'http://172.104.50.112:3001/testimonialFile/';

module.exports = function (app) {

	app.get('/webTestimonials/', function(req, res) {
		if (fs.existsSync(TESTIMONIAL_FILE) == true) {
				...
				if (err)
					...
					for (var ii = 0; ii < existingRows.length; ii++) {
						if (existingRows[ii].date == date) {
							var filesToDelete = existingRows[ii].fileInput.files;
					...
					fs.writeFileSync(TESTIMONIAL_FILE, JSON.stringify(newRows), 'utf8');
	...
	app.post('/webTestimonial/', function(req, res) {
		...
		var files = rowEntry.fileInput.files;
		for (var ii = 0; ii < files.length; ii++) {
			var fileToDownload = files[ii];
			var source = server + fileToDownload;
			var target = TESTIMONIAL_PATH + fileToDownload;
			var contentType = mime.lookup(fileToDownload);
			console.log('Downloading file: ' + target);
			request(source).pipe(fs.createWriteStream(target));
		}
		...
		if(fs.existsSync(TESTIMONIAL_FILE) == true) {
			fs.readFile(TESTIMONIAL_FILE, 'utf8', function(err, data) {
				if (err)
					...
					fs.writeFileSync(TESTIMONIAL_FILE, JSON.stringify(obj), 'utf8');
